...
- **Firefox**: сборка пока не подписана Mozilla, а обычный Firefox ставит только подписанные
  дополнения. Есть два способа:
  - откройте `about:debugging` → *Этот Firefox* → *Загрузить временное дополнение* и выберите
    `.xpi`. Дополнение проработает до перезапуска Firefox;
  - для постоянной установки возьмите Firefox Developer Edition или Nightly: в `about:config`
    поставьте `xpinstall.signatures.required` = `false`, затем откройте `.xpi`.
